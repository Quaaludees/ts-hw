...
function generateWords(number:number, words?:string[]):string {
    ...
    if (number < 20) {
        ...
        word = generateWords(Math.floor(number / NumbersEnum.ONE_HUNDRED)) + ' hundred';

    } else if (number < NumbersEnum.ONE_MILLION) {
        remainder = number % NumbersEnum.ONE_THOUSAND;
        word = generateWords(Math.floor(number / NumbersEnum.ONE_THOUSAND)) + ' thousand,';

    } else if (number < NumbersEnum.ONE_BILLION) {
        remainder = number % NumbersEnum.ONE_MILLION;
        word = generateWords(Math.floor(number / NumbersEnum.ONE_MILLION)) + ' million,';

    } else if (number < NumbersEnum.ONE_TRILLION) {
        remainder = number % NumbersEnum.ONE_BILLION;
        word = generateWords(Math.floor(number / NumbersEnum.ONE_BILLION)) + ' billion,';

    } else if (number < NumbersEnum.ONE_QUADRILLION) {
        remainder = number % NumbersEnum.ONE_TRILLION;
        word = generateWords(Math.floor(number / NumbersEnum.ONE_TRILLION)) + ' trillion,';

    } else if (number <= NumbersEnum.MAX) {
        remainder = number % NumbersEnum.ONE_QUADRILLION;
        word = generateWords(Math.floor(number / NumbersEnum.ONE_QUADRILLION)) +
            ' quadrillion,';
    }
